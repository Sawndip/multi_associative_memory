...
void percolation_test_parameters( 
	...
	double p_aff = 0.0; // Density of afferent edges
...
//Search for right density %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	int precision=6; // determine the correct density up to 10^-precision
	double increment=1;
		...
		while (average_active_ex < fidelity_outside){
			...
			average_active_ex=0;
				for(int j=0; j< number_trials_pattern; j++ ){
					int rand_bootstrap_pattern= rand()% num_patterns;
					//clog << "Shit\n";
					exposed = percolation::pattern_activation(LG.patterns_A[rand_bootstrap_pattern], act_threshold, z_i, 1, num_patterns);
					//clog << "Shit2\n";

					int pattern_active = Utils::count_active(LG.patterns_B[rand_bootstrap_pattern], act_threshold);
...
int main(){
	...
	double p_rec_increment= 0.01;
	double p_rec= 0 - p_rec_increment;

	int num_p_rec_values=101;
	for(int p_rec_iterator=0; p_rec_iterator< num_p_rec_values; p_rec_iterator++){
		p_rec += p_rec_increment;
		...
		int act_threshold = ceil(log(number_vertices)); // Activation threshold for excitatory vertices
			...
			for(int s_iterator=0; s_iterator< num_num_association_values; s_iterator++){
				...
				int number_trials_pattern=10;
				int number_trials_new_graph=1;
				int final_number_patterns_stored = (number_vertices/pattern_size)*(number_vertices/pattern_size);
				...
				file_percolation << "num_p_rec_values=" << num_p_rec_values << " ;\n";
				file_percolation << "p_rec_iterator="<< p_rec_iterator+1<< " ;\n";
				...
				clog << "Compute number of patterns for: \n ";
				clog << "p_rec=" << p_rec << "; \n";
				clog << "number_associations=" << number_associations << ";\n";
				

				percolation_test_parameters( 
				number_vertices, pattern_size, act_threshold,  p_rec, 
