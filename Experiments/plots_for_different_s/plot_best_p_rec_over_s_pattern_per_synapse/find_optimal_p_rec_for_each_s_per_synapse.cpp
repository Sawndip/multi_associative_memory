...
// Binary search for best recurrent density
	...
	for(int a=0; a< p_rec_precision; a++){
		...
		for(int b=0; b< number_p_rec_values ; b++){
			...
			maximal_number_pattern_values[b]= maximal_number_pattern_values[b]/((p_aff+p_rec)*number_vertices*number_vertices); // number of patterns per synapse
...
int main(){
	...
	for (int i=0; i < num_parallel;i++){
		...
		int number_associations=i+1;
		int num_num_association_values=num_parallel;
		int count_file=i+1;
