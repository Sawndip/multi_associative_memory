...
for j=1:number_files
    ...
    p_aff = 1;
    p_rec = 0.1*(j-1);
    rec_density_values(j) = 0.1*(j-1);
    aff_density_values(j) = 1;
    max_number_pattern(j) = mean(maximal_number_pattern_under_fidelity)*number_associations;
    ...
    real_compressed_benchmark(j) = log(2)*log2(number_vertices/log2(number_vertices))/log2(number_vertices)/number_associations
