...
// activated_vertices is the set of vertices activated in second layer at the end of the process.
...
// Voltage is not clamped and is reset at every round. 
void percolationwithinhibitionnonclamped(set<int>& activated_vertices, Twolayergraph& G, set<int> bootstrap, int act_threshold, int inhibition_threshold, int number_associations, int turn_strong_pattern_threshold) {

	int activated_vertices_in_this_round_size = 0;
	...
	for (vertexiterator = bootstrap.begin(); vertexiterator != bootstrap.end(); vertexiterator++) {
		...
		G.Firstlayer[*vertexiterator].voltage = act_threshold + 1;// do I need this?
		G.Firstlayer[*vertexiterator].activation_time = 1;
	}
	int current_round = 1;
	bool go_on = true;
	while(!verticesinqueue.empty() && go_on){
		int vertex_index = verticesinqueue.front();
		verticesinqueue.pop();
		int vertex_activation_round = verticesinqueue.front();
		verticesinqueue.pop();
		if (vertex_activation_round > current_round) {
			current_round++;
			//cout << current_round << "= current_round\n";
			// Reset voltages since new round has started!
			for (int i = 0; i < G.verticessecondlayer; i++) {
				G.Secondlayer[i].voltage = 0;
				//G.Secondlayer[i].activation_time = -1;
			}
			activated_vertices_in_this_round_size = activated_vertices.size();
			//cout << activated_vertices_in_this_round_size << " = vertices in this round\n";
			if (activated_vertices_in_this_round_size >= inhibition_threshold || current_round >= 10) {//200 is rather large - make it smaller after testing
				go_on = false;
			}
			else{
				activated_vertices.clear();
			}
		}
		if (vertex_activation_round == 1 && go_on) { // first round is only in first layer
			...
			for (int i = 0; i < neighbourhood_size; i++) {
				...
				if (G.Afferentedges[vertex_index][target_vertex].patternturnstrong < number_associations && G.Afferentedges[vertex_index][target_vertex].patternthreshold == turn_strong_pattern_threshold){// unnecessary to check whether edge is present
					...
					if (G.Secondlayer[target_vertex].voltage == act_threshold) {
						G.Secondlayer[target_vertex].voltage++;
						G.Secondlayer[target_vertex].activation_time = vertex_activation_round + 1;
		...
		else if(go_on){ // other rounds are only in second layer
			if (activated_vertices.size() < inhibition_threshold - 1) {
			...
			else if (activated_vertices.size() == inhibition_threshold - 1) { // Defines the final round
				activated_vertices.insert(vertex_index);
				//activated_vertices_size++;
				final_round = vertex_activation_round;
		...
		//should be unnecessary - activated_vertices will be empty if no vertex was added in current round to the queue
		//if (activated_vertices.size() < inhibition_threshold && verticesinqueue.empty()) {
		//	activated_vertices.clear();
		//}
	}
...
int percolation_inside_active_set_size(int pattern_size, int act_threshold, double p_aff, double p_rec) {
	...
	percolationwithinhibitionnonclamped(activated_vertices, G, Patternsfirstlayer[0].verticesinpattern, act_threshold, inhibition_threshold, 1, 1);
...
tuple<int,double,double, int> maximal_number_patterns_without_breaching_fidelity_outside_single_graph_trial (vector<Pattern>& Patternsfirstlayer, vector<Pattern>& Patternssecondlayer, vector<Pattern>& Patternsfirstlayernoisy, Twolayergraph& G, int number_trials_pattern) {
		...
		double act_threshold_down = G.act_threshold;
		...
		for (int j = 0; j < number_trials_pattern; j++) {
			...
			set<int> activated_vertices;
			percolationwithinhibitionnonclamped(activated_vertices, G, Patternsfirstlayernoisy[rand_bootstrap_pattern].verticesinpattern, G.act_threshold, inhibition_threshold, num_patterns, G.turn_strong_pattern_threshold);
...
/*
// Binary searches the optimal number of patterns which can be stored respecting fidelity outside. Single graph trial, should be averaged by calling it many times.
tuple<int,double,double> maximal_number_patterns_without_breaching_fidelity_outside_single_graph_trial (vector<Pattern>& Patternsfirstlayer, vector<Pattern>& Patternssecondlayer, vector<Pattern>& Patternsfirstlayernoisy, Twolayergraph& G, int number_trials_pattern) {
		...
		return make_tuple(num_patterns_down, average_active_vertices_down_final, average_active_vertices_inside_down_final);
}
*/
...
///////////////////////// TEST CODES ///////////////////////////////////////////////////////////////
	...
	percolationwithinhibitionnonclamped(activated_vertices, G, Patternsfirstlayer[test_pattern].verticesinpattern, act_threshold, inhibition_threshold, test_number_associations, turn_strong_pattern_threshold);
